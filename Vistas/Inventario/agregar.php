...
<?php
    ...
    if($_POST)
    {
        ...
        $producto = $controlador->Obtener($txtId);
        $fechadevencimientoactual = $producto['FechaDeVencimiento'];
        ...
        $fecha = date('Y-m-d');
        $fechadevencimiento = strtotime(str_replace('/','-',$fechadevencimientoactual));

        if($fechactual < $fechadevencimiento)
        {
            ...
            $Movimiento = new MovimientosInventario($idmovimiento,$cantidad,$fecha,$txtId,$movimiento);
        ...
        {
            $mensaje = "No se puede agregar existencias a un producto que ya esta vencido";
        }
    }
...
<div class="content-formularios">
    <h1 class="titulo">Agregar <?php echo $nombre; ?></h1>
